...
class UILIB_API Window : public virtual nbase::SupportWeakCallback {
  public:
	 ...
	 * @return 返回窗口句柄
	 */
	virtual HWND Create(HWND hwndParent, LPCTSTR pstrName, DWORD dwStyle,
						DWORD dwExStyle, bool isLayeredWindow = true,
  ...
  public:
	 ...
	 * @return 无
	 */
	void SetPos(const UiRect &rc, bool bNeedDpiScale, UINT uFlags,
				HWND hWndInsertAfter = NULL, bool bContainShadow = false);
	 ...
	 * @return 无
	 */
	void Resize(int cx, int cy, bool bContainShadow = false,
	 ...
	 * @return 返回消息处理结果
	 */
	LRESULT DoHandlMessage(UINT uMsg, WPARAM wParam, LPARAM lParam,
						   bool &handled);
	 ...
	 */
	inline bool HandleMouseEnterLeave(const POINT &pt, WPARAM wParam,
									  LPARAM lParam);
	 ...
	 * @return 控件指针
	 */
	Control *FindSubControlByClass(Control *pParent, const type_info &typeinfo,
								   int iIndex = 0);
	 ...
	 * @return 始终返回 true
	 */
	bool SendNotify(Control *pControl, EventType msg, WPARAM wParam = 0,
					LPARAM lParam = 0);
