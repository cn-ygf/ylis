...
class UILIB_API Control : public PlaceHolder {
  ...
  public:
	 ...
	 * @return 指定状态下设定的颜色字符串，对应 global.xml 中指定色值
	 */
	std::wstring GetStateBorderColor(ControlStateType stateType);
	 ...
	 * @return 无
	 */
	void SetStateBorderColor(ControlStateType stateType,
							 const std::wstring &strColor);
	 ...
	 * @return 返回控件的指针
	 */
	virtual Control *FindControl(FINDCONTROLPROC Proc, LPVOID pData,
								 UINT uFlags, CPoint scrollPos = CPoint());
	 ...
	 * @return 无
	 */
	void HandleMessageTemplate(EventType eventType, WPARAM wParam = 0,
							   LPARAM lParam = 0, TCHAR tChar = 0,
	 ...
	 */
	bool DrawImage(IRenderContext *pRender, Image &duiImage,
  ...
  protected:
	...
	virtual void PaintStatusBorderColor(IRenderContext *pRender);
