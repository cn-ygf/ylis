...
Control::Control()
	: OnXmlEvent(), OnEvent(), m_pUserDataBase(), m_bMenuUsed(false),
	  m_bEnabled(true), m_bMouseEnabled(true), m_bKeyboardEnabled(true),
	  m_bFocused(false), m_bMouseFocused(false), m_bSetPos(false),
	  m_bNoFocus(false), m_bClip(true), m_bGifPlay(true),
	  m_bReceivePointerMsg(true), m_bNeedButtonUpWhenKillFocus(false),
	  m_szEstimateSize(), m_renderOffset(), m_cxyBorderRound(), m_rcMargin(),
	  m_rcPaint(), m_rcBorderSize(), m_cursorType(kCursorArrow),
	  m_uButtonState(kControlStateNormal), m_nBorderSize(0),
	  m_nTooltipWidth(300), m_nAlpha(255), m_nHotAlpha(0), m_sToolTipText(),
	  m_sToolTipTextId(), m_sUserData(), m_strBkColor(), m_colorMap(),
	  m_strBorderColor(), m_gifWeakFlag(), m_animationManager(), m_imageMap(),
	  m_bkImage(), m_loadBkImageWeakFlag() {
	m_colorMap.SetControl(this);
	m_borderColorMap.SetControl(this);
	m_imageMap.SetControl(this);
	m_animationManager.Init(this);
}

Control::Control(const Control &r)
	: PlaceHolder(r), OnXmlEvent(), OnEvent(), m_pUserDataBase(),
	  m_bMenuUsed(r.m_bMenuUsed), m_bEnabled(r.m_bEnabled),
	  m_bMouseEnabled(r.m_bMouseEnabled),
	  m_bKeyboardEnabled(r.m_bKeyboardEnabled), m_bFocused(r.m_bFocused),
	  m_bMouseFocused(r.m_bMouseFocused), m_bSetPos(r.m_bSetPos),
	  m_bNoFocus(r.m_bNoFocus), m_bClip(r.m_bClip), m_bGifPlay(r.m_bGifPlay),
	  m_bReceivePointerMsg(r.m_bReceivePointerMsg),
	  m_bNeedButtonUpWhenKillFocus(r.m_bNeedButtonUpWhenKillFocus),
	  m_szEstimateSize(r.m_szEstimateSize), m_renderOffset(r.m_renderOffset),
	  m_cxyBorderRound(r.m_cxyBorderRound), m_rcMargin(r.m_rcMargin),
	  m_rcPaint(r.m_rcPaint), m_rcBorderSize(r.m_rcBorderSize),
	  m_cursorType(r.m_cursorType), m_uButtonState(kControlStateNormal),
	  m_nBorderSize(r.m_nBorderSize), m_nTooltipWidth(r.m_nTooltipWidth),
	  m_nAlpha(r.m_nAlpha), m_nHotAlpha(r.m_nHotAlpha),
	  m_sToolTipText(r.m_sToolTipText), m_sToolTipTextId(r.m_sToolTipTextId),
	  m_sUserData(r.m_sUserData), m_strBkColor(r.m_strBkColor),
	  m_colorMap(r.m_colorMap), m_borderColorMap(r.m_borderColorMap),
	  m_strBorderColor(r.m_strBorderColor), m_gifWeakFlag(),
	  m_animationManager(r.m_animationManager), m_imageMap(r.m_imageMap),
	  m_bkImage(r.m_bkImage), m_loadBkImageWeakFlag() {
	m_colorMap.SetControl(this);
	m_borderColorMap.SetControl(this);
	m_imageMap.SetControl(this);
	m_animationManager.Init(this);
...
std::wstring Control::GetBkColor() const { return m_strBkColor; }
...
std::wstring Control::GetStateBorderColor(ControlStateType stateType) {
	return m_borderColorMap[stateType];
}

void Control::SetStateBorderColor(ControlStateType stateType,
								  const std::wstring &strColor) {
	ASSERT(GlobalManager::GetTextColor(strColor) != 0);
	if (m_borderColorMap[stateType] == strColor)
		return;
...
ControlStateType Control::GetState() const { return m_uButtonState; }
...
int Control::GetBorderSize() const { return m_nBorderSize; }
...
std::wstring Control::GetBorderColor() const { return m_strBorderColor; }
...
int Control::GetLeftBorderSize() const { return m_rcBorderSize.left; }
...
int Control::GetTopBorderSize() const { return m_rcBorderSize.top; }
...
int Control::GetRightBorderSize() const { return m_rcBorderSize.right; }
...
int Control::GetBottomBorderSize() const { return m_rcBorderSize.bottom; }
...
CSize Control::GetBorderRound() const { return m_cxyBorderRound; }
...
CursorType Control::GetCursorType() const { return m_cursorType; }

void Control::SetCursorType(CursorType flag) { m_cursorType = flag; }
...
int Control::GetToolTipWidth(void) const { return m_nTooltipWidth; }

bool Control::IsContextMenuUsed() const { return m_bMenuUsed; }

void Control::SetContextMenuUsed(bool bMenuUsed) { m_bMenuUsed = bMenuUsed; }

std::wstring Control::GetDataID() const { return m_sUserData; }
...
void Control::SetDataID(const std::wstring &strText) { m_sUserData = strText; }
...
UserDataBase *Control::GetUserDataBase() const { return m_pUserDataBase.get(); }
...
bool Control::IsEnabled() const { return m_bEnabled; }
...
bool Control::IsMouseEnabled() const { return m_bMouseEnabled; }

void Control::SetMouseEnabled(bool bEnabled) { m_bMouseEnabled = bEnabled; }

bool Control::IsKeyboardEnabled() const { return m_bKeyboardEnabled; }
void Control::SetKeyboardEnabled(bool bEnabled) {
	m_bKeyboardEnabled = bEnabled;
}

bool Control::IsFocused() const { return m_bFocused; }
...
UINT Control::GetControlFlags() const { return UIFLAG_TABSTOP; }

void Control::SetNoFocus() { m_bNoFocus = true; }
...
UiRect Control::GetMargin() const { return m_rcMargin; }
...
void Control::HandleMessageTemplate(EventType eventType, WPARAM wParam,
									LPARAM lParam, TCHAR tChar, CPoint mousePos,
									FLOAT pressure) {
	EventArgs msg;
	msg.pSender = this;
	msg.Type = eventType;
...
void Control::HandleMessage(EventArgs &msg) {
	if (!IsMouseEnabled() && msg.Type > kEventMouseBegin &&
		...
		MouseLeave(msg);
			   ...
			   msg.Type == kEventInternalDoubleClick) {
		ButtonDown(msg);
		return;
...
void Control::SetAttribute(const std::wstring &strName,
	...
	if (strName == _T("class")) {
		...
		SetStateBorderColor(kControlStateNormal, strValue);
	else if (strName == _T("hotbordercolor"))
		SetStateBorderColor(kControlStateHot, strValue);
	else if (strName == _T("pushedbordercolor"))
		SetStateBorderColor(kControlStatePushed, strValue);
	else if (strName == _T("disabledbordercolor"))
		SetStateBorderColor(kControlStateDisabled, strValue);
...
void Control::GetImage(Image &duiImage) const {
	...
	if (::PathIsRelative(sImageName.c_str())) {
		imageFullPath = GlobalManager::GetResourcePath() +
						m_pWindow->GetWindowResourcePath() + sImageName;
	}
...
bool Control::DrawImage(IRenderContext *pRender, Image &duiImage,
	...
	if (m_bkImage.imageCache && m_bkImage.imageCache->IsGif() && m_bGifPlay &&
		...
		pRender->DrawImage(
			...
			rcNewDest, rcNewSource, newImageAttribute.rcCorner, iFade,
			newImageAttribute.bTiledX, newImageAttribute.bTiledY);
	}
...
void Control::AlphaPaint(IRenderContext *pRender, const UiRect &rcPaint) {
	...
	if (IsAlpha()) {
		...
		AutoClip roundClip(pRender, m_rcItem, m_cxyBorderRound.cx,
						   m_cxyBorderRound.cy, bRoundClip);
...
void Control::Paint(IRenderContext *pRender, const UiRect &rcPaint) {
	...
	PaintText(pRender);
	PaintStatusBorderColor(pRender);
	// PaintBorder(pRender);
}
...
void Control::PaintStatusBorderColor(IRenderContext *pRender) {
	if (m_rcBorderSize.left > 1) {
		int i = 0;
	}
	m_borderColorMap.PaintStatusBorderColor(pRender, m_rcItem, m_rcBorderSize,
											m_uButtonState);
}
...
void Control::PaintText(IRenderContext *pRender) { return; }
