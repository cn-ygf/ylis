...
Control::Control(const Control& r) :
	...
	m_borderColorMap(r.m_borderColorMap),
...
void Control::SetStateBorderColor(ControlStateType stateType, const std::wstring& strColor)
{
	ASSERT(GlobalManager::GetTextColor(strColor) != 0);
...
{
	// 判断本控件是否有hot状态
	return m_colorMap.HasHotColor() || m_imageMap.HasHotImage() || m_borderColorMap.HasHotColor();
}
...
void Control::SetAttribute(const std::wstring& strName, const std::wstring& strValue)
{
	...
	else if (strName == _T("normalbordercolor")) SetStateBorderColor(kControlStateNormal, strValue);
	else if (strName == _T("hotbordercolor")) SetStateBorderColor(kControlStateHot, strValue);
	else if (strName == _T("pushedbordercolor")) SetStateBorderColor(kControlStatePushed, strValue);
	else if (strName == _T("disabledbordercolor")) SetStateBorderColor(kControlStateDisabled, strValue);
...
void Control::PaintStatusBorderColor(IRenderContext* pRender)
{
	...
	m_borderColorMap.PaintStatusBorderColor(pRender, m_rcItem, m_rcBorderSize, m_uButtonState);
}
