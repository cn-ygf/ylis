...
ImageInfo::ImageInfo()
	: m_bAlphaChannel(false), m_bCached(false), m_propertyItem(),
	  m_vecBitmap() {}
...
ImageInfo::LoadImage(const std::wstring &strImageFullPath) {
...
std::unique_ptr<ImageInfo>
ImageInfo::LoadImage(HGLOBAL hGlobal, const std::wstring &strImageFullPath) {
	if (SvgUtil::IsSvgFile(strImageFullPath))
...
Image::Image()
	: imageAttribute(), imageCache(), m_nCurrentFrame(0), m_bPlaying(false),
	  m_nCycledCount(0) {}
...
bool StateImageMap::PaintStatusImage(
	IRenderContext *pRender, StateImageType stateImageType,
	ControlStateType stateType, const std::wstring &sImageModify /*= L""*/) {
...
void StateColorMap::PaintStatusBorderColor(IRenderContext *pRender,
										   UiRect rcItem, UiRect rcBorderSize,
										   ControlStateType stateType) {
	/*if (m_pControl) {
		bool bFadeHot =
	m_pControl->GetAnimationManager().GetAnimationPlayer(kAnimationHot) !=
	nullptr; int nHotAlpha = m_pControl->GetHotAlpha(); if (bFadeHot) { if
	((stateType == kControlStateNormal || stateType == kControlStateHot)
...
void StateColorMap::PaintBorder(IRenderContext *pRender, UiRect rcItem,
								UiRect rcBorderSize,
								ControlStateType stateType) {
	...
	if (dwBorderColor != 0) {
		if (rcBorderSize.left > 0 || rcBorderSize.top > 0 ||
			...
			if (rcBorderSize.left > 0) {
				...
				rcBorder.right = rcBorder.left =
					rcItem.left + rcBorderSize.left / 2;
			...
			if (rcBorderSize.right > 0) {
				...
				rcBorder.left = rcBorder.right =
					rcItem.right - (rcBorderSize.right + 1) / 2;
			...
			if (rcBorderSize.bottom > 0) {
				...
				rcBorder.top = rcBorder.bottom =
					rcItem.bottom - (rcBorderSize.bottom + 1) / 2;
		...
		dwBorderColor); else pRender->DrawRect(rcDraw, m_nBorderSize,
		dwBorderColor);
		}*/
	}
}
		...
		if (bFadeHot) {
			if ((stateType == kControlStateNormal ||
				...
				pRender->DrawColor(rcPaint,
								   m_stateColorMap[kControlStateNormal]);
				if (nHotAlpha > 0) {
					pRender->DrawColor(
						rcPaint, m_stateColorMap[kControlStateHot], nHotAlpha);
				}
				return;
			}
		}
